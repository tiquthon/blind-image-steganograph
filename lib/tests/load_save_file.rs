...
fn load_save_file() {
    ...
    let result_file_a = temp_dir.child("test_a.jpg");
    let result_file_b = temp_dir.child("test_b.jpg");

    // Act
    let image_a = Image::load_from_file("kek.webp");

    image_a.save_to_file(&result_file_a, ImageFormat::WebP);

    let image_a_bytes = std::fs::read(&result_file_a).unwrap();
    let image_b = Image::load_from_memory(&image_a_bytes);

    let image_b_bytes = image_b.save_to_memory(ImageFormat::WebP);

    std::fs::write(&result_file_b, &image_b_bytes).unwrap();
    let image_c = Image::load_from_file_with_format(&result_file_b, ImageFormat::WebP);

    let image_d = Image::load_from_memory_with_format(&image_b_bytes, ImageFormat::WebP);

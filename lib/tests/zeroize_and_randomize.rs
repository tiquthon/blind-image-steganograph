...
fn zeroize_works() {
    // Arrange
    let (text_pixel_x, test_pixel_y, test_pixel_value_initial) = {
        let image = image::open("tests/assets/pexels-pixabay-53114_resized.webp")
            .unwrap();
    ...
    image.save_to_file(&temp_file, ImageFormat::WebP);

    let test_pixel_value = image::open(&temp_file)
        .unwrap()
        .get_pixel(text_pixel_x, test_pixel_y);
